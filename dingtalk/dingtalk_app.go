...
func (Self *TDingTalkApp) GetUserInfoByUnionId(unionid string) (*TDDUser, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("发送请求失败，原因：%s", err.Error()))
	} //{"errcode":0,"errmsg":"ok","contactType":0,"userid":"201"}
	...
	err = req.ToJSON(&info)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("序列化接收消息失败，内容：%s，原因：%s", body_text, err.Error()))
	}
...
func (Self *TDingTalkApp) GetUserInfo(userid string) (*TDDUser, error) {
	...
	if info.ErrCode == 0 {
		//logs.Debug("GetUserInfo() : 获取钉钉用户信息 ...... %s-%s", info.StaffCode, info.StaffName)
		return &info, nil
	} else if info.ErrCode == 503 {
		Self.token = nil
...
func (Self *TDingTalkApp) SendWorkNotify(user_id string, msg_text string) (int, error) {
	//logs.Debug("SendWorkNotify() : 发送钉钉工作通知消息")
	_, err := Self.GetAccessToken()
	if err != nil {
		return -1, err
	}
	...
	body_text, err := req.String()
	if err != nil {
		return -1, errors.New(fmt.Sprintf("发送钉钉工作通知消息失败，原因：%s", err.Error()))
	}

	var info workNotify
	err = req.ToJSON(&info)
	if err != nil {
		return -1, errors.New(fmt.Sprintf("发送钉钉工作通知消息失败，内容：%s，原因：%s", body_text, err.Error()))
	}
	if info.ErrCode == 0 {
