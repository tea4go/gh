...
func (p *Parser) parseTag() (string, int, error) {
	var b byte
	var pid_b, pid_e, pid_b_f, tag_e bool
	var tag []byte
	var pid int
	var err error
	...
	for {
		...
		if pid_b_f && pid_e {
			pid, err = strconv.Atoi(string(p.buff[from:p.cursor]))
			if err != nil {
				pid = 0
			}
			p.cursor++
	...
	//fmt.Println(string(tag), pid, p.cursor, err)
