...
func ParsePacket(data, secret []byte, dictionary *TDictionary) (*TDataPacket, error) {
	if len(data) < 20 {
		return nil, errors.New(fmt.Sprintf("有效包必须大于20字节。 目前包大小[%d]", len(data)))
	}
	...
	if length < 20 || length > maxPacketSize {
		return nil, errors.New(fmt.Sprintf("无效包长度%d(20-%d)", length, maxPacketSize))
	}
	...
	for len(attributes) > 0 {
		if len(attributes) < 2 {
			return nil, errors.New(fmt.Sprintf("有效属性必须大于2字节。 BufferSize=%d", len(attributes)))
		}

		attrLength := attributes[1]
		if attrLength < 1 || attrLength > 253 || len(attributes) < int(attrLength) {
			return nil, errors.New(fmt.Sprintf("无效属性长度%d(2-%d)", attrLength, 253))
		}
		...
		if err != nil {
			if attrType == 2 { //User-Password,如果密码解析不了，则使用默认密码123123
				...
				return nil, errors.New(fmt.Sprintf("解码属性(%d)失败，数据[%s]，%s", attrType, hexbuff[:b], err.Error()))
			}
		} else {
			attr := &TAttribute{
				AttrId:    attrType,
...
func (p *TDataPacket) PAP() (username, password string, err error) {
	if p.Code != CodeAccessRequest {
		err = errors.New(fmt.Sprintf("只接收AccessRequest(Code=%d)请求包，当前为(Code=%d)", CodeAccessRequest, p.Code))
		return
	}
