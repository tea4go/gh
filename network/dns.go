...
func GetIPByDomain(query_name string, args ...interface{}) (string, []string, string, error) {
	...
	if ip_port != "" {
		...
		if utils.FileIsExist(resolv_name) {
			...
			if err != nil {
				return query_name, nil, ip_port, errors.New(fmt.Sprintf("解析系统DNS配置失败，原因：%s\n", err.Error()))
			}
			if len(config.Servers) == 0 {
				return query_name, nil, ip_port, errors.New(fmt.Sprintf("解析系统DNS配置失败，文件%s没有配置DNS服务器，请手工指定。\n", resolv_name))
			}
	...
	if r == nil {
		return query_name, nil, ip_port, errors.New(fmt.Sprintf("解析域名(%s)失败。 \n原因：%s\n", query_name, err.Error()))
	}

	if r.Rcode != dns.RcodeSuccess {
		return query_name, nil, ip_port, errors.New(fmt.Sprintf("解析域名(%s)失败，错误码：%d\n", query_name, r.Rcode))
	}
