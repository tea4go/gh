...
func GetMacByIP_file(ip_text string) (string, error) {
	...
	if err != nil {
		return "", errors.New(fmt.Sprintf("没有找到IP对应的Mac地址。"))
	}
	if err != nil {
		return "", errors.New(fmt.Sprintf("执行Arp命令出错，原因：%s", cmd_out_text))
	}

	reg_text := `([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}`
	reg, err := regexp.Compile(reg_text)
	if err != nil {
		return "", errors.New(fmt.Sprintf("正则表达式错误，表达式：%s，原因：%s", reg_text, err.Error()))
	}
...
func GetMacByIP_arp(ip_text string) (string, error) {
	...
	for _, inter := range interfaces {
		...
		if err != nil {
			return "", errors.New(fmt.Sprintf("创建Arp网络句柄出错，原因：%s", err.Error()))
		}
		defer func() {
		...
		if err != nil {
			return "", errors.New(fmt.Sprintf("设置网络超时出错，原因：%s", err.Error()))
		}
		...
		if err != nil {
			//return "", errors.New(fmt.Sprintf("获得Mac出错，原因：%s", err.Error()))
			continue
		}
