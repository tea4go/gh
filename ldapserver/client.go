...
func (c *client) serve() {
	...
	if onc := c.srv.onNewConnection; onc != nil {
		if err := onc(c.rwc); err != nil {
			logs.Debug("客户端[%d]： OnNewConnection错误(%s)", c.Numero, err)
			return
		}
	}
	...
	for {
		...
		if err != nil {
			if opErr, ok := err.(*net.OpError); ok && opErr.Timeout() {
				logs.Debug("客户端[%d]： 读取数据超时(%s)", c.Numero, err)
			} else {
				logs.Debug("客户端[%d]： 读取数据错误(%s)", c.Numero, err)
			}
			return
		}
		...
		if err != nil {
			logs.Debug("客户端[%d]： 解析数据错误(%s)", c.Numero, err)
			continue
		}
		logs.Debug("客户端[%d]： <<< %s", c.Numero, message.ProtocolOpName())
...
func (c *client) close() {
	logs.Debug("客户端[%d]%s断开连接 ......", c.Numero, c.Addr().String())
	close(c.closing)
...
func (c *client) writeMessage(m *ldap.LDAPMessage) {
	data, _ := m.Write()
	//logs.Debug("client [%d]: >>> %s", c.Numero, m.ProtocolOpName())
