...
func (e *FileEvent) GetMask() string {
	...
	if (mask & sys_FS_CREATE) == sys_FS_CREATE {
		events += "|" + "FS_CREATE"
		mask = mask ^ sys_FS_CREATE
	}
	if (mask & sys_FS_DELETE) == sys_FS_DELETE {
		events += "|" + "FS_DELETE"
		mask = mask ^ sys_FS_DELETE
	}
	if (mask & sys_FS_DELETE_SELF) == sys_FS_DELETE_SELF {
		events += "|" + "FS_DELETE_SELF"
		mask = mask ^ sys_FS_DELETE_SELF
	}

	if (mask & sys_FS_MODIFY) == sys_FS_MODIFY {
		events += "|" + "FS_MODIFY"
		mask = mask ^ sys_FS_MODIFY
	}
	if (mask & sys_FS_ATTRIB) == sys_FS_ATTRIB {
		events += "|" + "FS_ATTRIB"
		mask = mask ^ sys_FS_ATTRIB
	}
	if (mask & sys_FS_MOVE) == sys_FS_MOVE {
		events += "|" + "FS_MOVE"
		mask = mask ^ sys_FS_MOVE
	}

	if (mask & sys_FS_MOVE_SELF) == sys_FS_MOVE_SELF {
		events += "|" + "FS_MOVE_SELF"
		mask = mask ^ sys_FS_MOVE_SELF
	}
	if (mask & sys_FS_MOVED_FROM) == sys_FS_MOVED_FROM {
		events += "|" + "FS_MOVED_FROM"
		mask = mask ^ sys_FS_MOVED_FROM
	}
	if (mask & sys_FS_MOVED_TO) == sys_FS_MOVED_TO {
		events += "|" + "FS_MOVED_TO"
		mask = mask ^ sys_FS_MOVED_TO
	}

	if (mask & sys_FS_CLOSE) == sys_FS_CLOSE {
		events += "|" + "FS_CLOSE"
		mask = mask ^ sys_FS_CLOSE
	}
