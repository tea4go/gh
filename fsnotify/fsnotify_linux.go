...
func (e *FileEvent) GetMask() string {
	...
	if (mask & sys_IN_ATTRIB) == sys_IN_ATTRIB {
		events += "|" + "IN_ATTRIB"
		mask = mask ^ sys_IN_ATTRIB
	}

	if (mask & sys_IN_CLOSE) == sys_IN_CLOSE {
		events += "|" + "IN_CLOSE"
		mask = mask ^ sys_IN_CLOSE
	}
	if (mask & sys_IN_CLOSE_WRITE) == sys_IN_CLOSE_NOWRITE {
		events += "|" + "IN_CLOSE_NOWRITE"
		mask = mask ^ sys_IN_CLOSE_NOWRITE
	}
	if (mask & sys_IN_CLOSE_WRITE) == sys_IN_CLOSE_WRITE {
		events += "|" + "IN_CLOSE_WRITE"
		mask = mask ^ sys_IN_CLOSE_WRITE
	}

	if (mask & sys_IN_CREATE) == sys_IN_CREATE {
		events += "|" + "IN_CREATE"
		mask = mask ^ sys_IN_CREATE
	}

	if (mask & sys_IN_DELETE) == sys_IN_DELETE {
		events += "|" + "FS_DELETE"
		mask = mask ^ sys_IN_DELETE
	}
	if (mask & sys_IN_DELETE_SELF) == sys_IN_DELETE_SELF {
		events += "|" + "FS_DELETE_SELF"
		mask = mask ^ sys_IN_DELETE_SELF
	}

	if (mask & sys_IN_MODIFY) == sys_IN_MODIFY {
		events += "|" + "IN_MODIFY"
		mask = mask ^ sys_IN_MODIFY
	}

	if (mask & sys_IN_MOVE) == sys_IN_MOVE {
		events += "|" + "IN_MOVE"
		mask = mask ^ sys_IN_MOVE
	}
	if (mask & sys_IN_MOVE_SELF) == sys_IN_MOVE_SELF {
		events += "|" + "IN_MOVE_SELF"
		mask = mask ^ sys_IN_MOVE_SELF
	}
	if (mask & sys_IN_MOVED_FROM) == sys_IN_MOVED_FROM {
		events += "|" + "IN_MOVED_FROM"
		mask = mask ^ sys_IN_MOVED_FROM
	}
	if (mask & sys_IN_MOVED_TO) == sys_IN_MOVED_TO {
		events += "|" + "IN_MOVED_TO"
		mask = mask ^ sys_IN_MOVED_TO
	}
	if (mask & sys_IN_OPEN) == sys_IN_OPEN {
		events += "|" + "IN_OPEN"
		mask = mask ^ sys_IN_OPEN
	}
	...
	if mask > 0 {
		events += fmt.Sprintf("|未知操作(%d)", mask)
	}
